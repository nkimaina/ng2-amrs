...
export class VisitStarterComponent implements OnInit {
  ...
  constructor(private userDefaultPropertiesService: UserDefaultPropertiesService,
  ...
  getCurrentProgramEnrollmentConfig() {
    if (this.programEnrollmentUuid === '') {
      this.programEnrollmentUuid = 'e0bd4d29-52f8-4e8c-b531-cd928b0f753b';
    }
    this.isBusy = true;
    ...
    this.patientProgramResourceService
      ...
      (error) => {
        this.isBusy = false;
        this.error = 'Error loading the program visit configs';
